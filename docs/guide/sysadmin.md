...
```
1.在CPU控制器目录下创建子目录，下面的目录代表一个控制组，用户可以在其中放置特定进
...
4.将需要限制的PID加入cgroup任务,本例为10616
```
[root@OpencloudOS~]# echo "10616" > /sys/fs/cgroup/cpu/stress/cgroup.procs 
```
5.查看该进程的CPU占用是否限制在50%以内
 ``` 
