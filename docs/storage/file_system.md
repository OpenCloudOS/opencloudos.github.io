...
# 第 33 章使用 IdM 的 SSSD 组件缓存 autofs 映射
...
1. 编辑 `/etc/autofs.conf` 文件以指定 `autofs` 搜索的架构属性：

   ```
   #
   # 其他常见的 LDAP 命名
   #
   map_object_class = "自动挂载地图"
   entry_object_class = "自动挂载"
   map_attribute = "automountMapName"
   entry_attribute = "自动挂载键"
   value_attribute = "自动挂载信息"
   ```
...
3. 编辑 /etc/autofs_ldap_auth.conf 文件，以便 autofs 允许使用 IdM LDAP 服务器进行客户端身份验证。

  - 将 `authrequired` 更改为 yes。
  - 将主体设置为 IdM LDAP 服务器的 Kerberos 主机主体，host/fqdn@REALM。主体名称用于连接到 IdM 目录，作为 GSS 客户端身份验证的一部分。
...
（可选）设置 autofs 条目的搜索库。默认情况下，这是 LDAP 搜索库，但可以在 ldap_autofs_search_base 参数中指定子树。
...
# systemctl 重启 sssd.service
检查 /etc/nsswitch.conf 文件，以便将 SSSD 列为自动挂载配置的源：

自动挂载：sss 文件
重启 autofs 服务：

# systemctl 重启 autofs.service
通过列出用户的 /home 目录来测试配置，假设 /home 有一个主映射条目：

# ls /home/用户名
如果这没有挂载远程文件系统，请检查 /var/log/messages 文件是否有错误。如有必要，通过将日志记录参数设置为调试来提高 /etc/sysconfig/autofs 文件中的调试级别。